...
export async function GET(
    ...
    const { chapter: chStr, verse: vStr } = await ctx.params; // ✅ unwrap
    ...
    const vRows = await sql`
        ...
        from verses
        where book = 'Psalms' and chapter = ${chapter} and verse = ${verse}
            limit 1
    `;
    ...
    let aRows = await sql`
        ...
        from verse_analyses
        where verse_id = ${verseId} and analysis_version = ${effectiveVersion}
            limit 1
    `;
