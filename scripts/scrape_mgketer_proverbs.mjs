// scripts/scrape_mgketer_proverbs.mjs
import fs from "fs";
...
function parseChapterText(html, chapter) {
    // מחלץ הכל בין [א] ... [ב] ... וכו'
    const text = html
        .replace(/\s+/g, " ")
        .replace(/<[^>]+>/g, " "); // strip tags

    const re =
        /\[([\u05D0-\u05EA"׳״]{1,6})\]\s*(.+?)(?=\s*\[[\u05D0-\u05EA"׳״]{1,6}\]\s*|$)/g;
...
async function main() {
    const BOOK_ID = 28;           // משלי
    const MAX_CHAPTER = 31;       // משלי = 31 פרקים
    const OUT_FILE = "proverbs-mgketer.json";

    const all = [];

    for (let chapter = 1; chapter <= MAX_CHAPTER; chapter++) {
        const url = `https://www.mgketer.org/mikra/${BOOK_ID}/${chapter}`;
    ...
    console.log(`✅ total verses: ${all.length} (written to ${OUT_FILE})`);
}
